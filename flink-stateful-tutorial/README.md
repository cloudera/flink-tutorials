...
The actual `TransactionProcessor` instance provided at the ConnectedStreams.process() call is not the one that will be used, instead it will be serialized, and later deserialized in order to have one instance for each parallel stream processor. In other words the one created is a template for the future instances.
